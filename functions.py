...
class Panel:
    ...
    def eta(self,alpha):
            return 1/(self.nup(alpha)-self.zeta(alpha)/2)
    
    def kapa(self,alpha):
        return (1+2*self.a2d(alpha))/(self.nu(alpha)-2*self.d*self.zeta(alpha))*self.kapa0
   
    def mu(self,alpha):
        return 2*self.zeta(alpha)* (self.kapa(alpha)/self.kapa0+self.a2d(alpha)/(self.d*self.zeta(alpha)))*self.kapa0/(2*self.nu(alpha)/self.d-3*self.zeta(alpha))
